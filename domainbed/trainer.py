...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    ...
    if target_env is not None:
        assert target_env in test_envs
    ...
    for i, (env, _) in enumerate(in_splits + out_splits + test_splits):
        ...
        loader_kwargs = {"dataset": env, "batch_size": batchsize, "num_workers": dataset.N_WORKERS, "worker_init_fn": _seed_worker}
    ...
    stopper = misc.EarlyStopping(patience=-1)
    ...
    for step in range(n_steps):
        ...
        inputs = {**batches, "step": step, "epoch": step // steps_per_epoch}
        ...
        if step % checkpoint_freq == 0 or stopper.possible_stop or step == n_steps - 1:
            ...
            accuracies, summaries = evaluator.evaluate(algorithm, ret_domain=hparams["domain_eval"])
            results["eval_t"] = time.time() - eval_start_time
            ...
            writer.add_scalars_with_prefix(summaries, step, f"{testenv_name}/label/summary/")
            writer.add_scalars_with_prefix(accuracies, step, f"{testenv_name}/label/all/")

            fi = summaries[selection]
            if fi > best:
                    best = fi
            ...
            if args.model_save:
                if best == fi:
                    ...
                    filename = "TE{}_{}_best.pth".format(test_env_str, args.trial_seed)
                    ...
                    if not args.debug:
                        ...
                        # logger.info(f"Model saved at step: {step}")
                    else:
            ...
            #         "test_envs": test_envs,
            #         "model_dict": algorithm.cpu().state_dict(),
            #         "plot_data": algorithm.plot_data()
            #     }
            #     algorithm.cuda()
            #     torch.save(save_dict, path)
            #     algorithm.vis(args.out_dir)
            #     if not args.debug:
            #         torch.save(save_dict, path)
            #     else:
            #         logger.debug("DEBUG Mode -> no save (org path: %s)" % path)
            ...
            if stopper(it=step, fitness=fi):
                logger.info(f'Stopping training early as no improvement observed in last {stopper.patience} validation steps. '
                                        f'Best results observed at iteration {stopper.best_iter}')
                break
    ...
    iid_best_train_out = records.argmax("train_out")
    # iid_best = (iid_best_train_out["test_in"] * (1 - args.holdout_fraction)) + (iid_best_train_out["test_out"] * args.holdout_fraction)
    ...
    if hparams["domain_eval"]:
        domain_acc_iid = records.argmax("train_out")["dm_acc"]
        ret["domain"] = domain_acc_iid

    metrics = {
        "oracle": oracle_best,
        "ood": iid_best,
        "iid": iid_best_indomain
    }
    writer.add_scalars_with_prefix(metrics, 0, f"{testenv_name}/metrics/")
