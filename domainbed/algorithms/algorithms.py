...
from domainbed.lib import visualize
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, logger):
...
class Backbone(nn.Module):
    def __init__(self, input_shape, output_shape, hparams) -> None:
        super().__init__()

        self.features = networks.Featurizer(input_shape, hparams)
        self.classifier = nn.Linear(self.features.n_outputs, output_shape)

    def forward(self, x):
        return self.classifier(self.features(x))

class ERMDAdv(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams, logger):
        super().__init__(input_shape, num_classes, num_domains, hparams)

        self.label_clf = Backbone(input_shape=input_shape, output_shape=num_classes, hparams=self.hparams)
        self.opt_label = torch.optim.Adam(self.label_clf.parameters(),
                lr=self.hparams["lr"], weight_decay=self.hparams['weight_decay'])

        self.hparams["resnet18"] = True
        self.domain_clf = Backbone(input_shape=input_shape, output_shape=num_domains, hparams=self.hparams)
        self.opt_domain = torch.optim.Adam(self.domain_clf.parameters(),
                lr=self.hparams["lr"], weight_decay=self.hparams['weight_decay'])

        ema_rate = 0.999
        avg_fn = lambda averaged_model_parameter, model_parameter, num_averaged: \
                    ema_rate * averaged_model_parameter + (1 - ema_rate) * model_parameter
        
        self.ema_domain = torch.optim.swa_utils.AveragedModel(self.domain_clf, avg_fn=avg_fn)
        for ema_p in self.ema_domain.parameters():
            ema_p.requires_grad_(False)
        self.ema_domain.train()

    @property
    def optimizer(self):
        return [self.opt_label, self.opt_domain]

    @torch.no_grad()
    def pre_update(self, all_x, all_y, domians):
        all_x = self.look_ahead(all_x, all_y, domians)
        return all_x

    @torch.no_grad()
    def look_ahead(self, all_x, all_y, domians):
        
        self.label_clf.eval()
        self.domain_clf.eval()
        self.ema_domain.eval()

        loss_adv = []

        BS, N_SAMPLES, N_CHANNELS, HEIGHT, WIDTH = all_x.shape

        for ix in range(N_SAMPLES):
            adv_student = F.cross_entropy(self.domain_clf(all_x[:, ix]),
                                           domians, reduction='none')
            adv_teacher = F.cross_entropy(self.ema_domain(all_x[:, ix]),
                                           domians, reduction='none')

            ll = self.hparams['alpha'] * adv_student - (1 - self.hparams['alpha']) * adv_teacher
            loss_adv.append(ll)

        loss_adv = torch.stack(loss_adv)
        s1 = loss_adv.topk(1, dim=0).indices.squeeze(0)

        self.label_clf.train()
        self.domain_clf.train()
        self.ema_domain.train()

        return all_x[range(BS), s1[0]].view(-1, N_CHANNELS, HEIGHT, WIDTH)
    ...
    def update(self, x, y, step, **kwargs):
        ...
        all_x = self.pre_update(all_x, all_y, domains)

        loss_domain = F.cross_entropy(self.domain_clf(all_x), domains)
        self.opt_domain.zero_grad()
        loss_domain.backward()
        self.opt_domain.step()
        self.ema_domain.update_parameters(self.domain_clf)

        loss_label = F.cross_entropy(self.label_clf(all_x), all_y)
        self.opt_label.zero_grad()
        loss_label.backward()
        self.opt_label.step()

        return {'loss': loss_label.item()}

    def predict(self, x):
        return self.label_clf(x)

class ERMAdv(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, logger):
        ...
        self.ema_network = torch.optim.swa_utils.AveragedModel(self.network, avg_fn=avg_fn)
        for ema_p in self.ema_network.parameters():
            ema_p.requires_grad_(False)
        self.ema_network.train()

    @torch.no_grad()
    def pre_update(self, all_x, all_y):
        if self.hparams['reward'] == 'label':
            all_x = self.look_ahead(all_x, all_y)
        else:
            all_x = torch.cat([all_x[:, 0], all_x[:, 1]])

        return all_x

    @torch.no_grad()
    def look_ahead(self, all_x, all_y):
        ...
        for ix in range(N_SAMPLES):
            adv_student = F.cross_entropy(self.network(all_x[:, ix]),
                                           all_y, reduction='none')
            adv_teacher = F.cross_entropy(self.ema_network(all_x[:, ix]),
                                           all_y, reduction='none')
    ...
    def update(self, x, y, step, **kwargs):
        ...
        all_x = self.pre_update(all_x, all_y)
        
        if self.hparams["reward"] == "label":
            loss = F.cross_entropy(self.network(all_x), all_y)
        else:
            weak_loss = F.cross_entropy(self.network(all_x[:BS]), all_y)
            strong_loss = F.cross_entropy(self.network(all_x[BS:]), all_y)
            loss  = self.hparams["alpha"] * strong_loss + (1 - self.hparams["alpha"]) * weak_loss
...
class IRM(ERM):
    ...
    def update(self, x, y, **kwargs):
        ...
        for i, (x, y) in enumerate(minibatches):
            ...
            penalty += self._irm_penalty(logits, y, self.hparams['device'])
        nll /= len(minibatches)
...
class CutMix(ERM):
    ...
    def update(self, x, y, **kwargs):
        ...
        if self.hparams["beta"] > 0 and r < self.hparams["cutmix_prob"]:
            ...
            rand_index = torch.randperm(x.size()[0]).to(self.hparams['device'])
            target_a = y
...
class SagNet(Algorithm):
    ...
    def randomize(self, x, what="style", eps=1e-5):
        sizes = x.size()
        alpha = torch.rand(sizes[0], 1).to(self.hparams['device'])
...
class RSC(ERM):
    ...
    def update(self, x, y, **kwargs):
        ...
        mask_f = all_g.lt(percentiles.to(self.hparams['device'])).float()
