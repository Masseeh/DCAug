...
def _get_scheduler(name, optimizer, lr, total_steps, final_div_factor=1e4):
    if name == "onecycle":
        ...
        scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[4000], gamma=0.1)
...
def get_scheduler(name, optimizer, lr, total_steps, final_div_factor=1e4):
    ...
    if isinstance(optimizer, list):
        for opt in optimizer:
            schedulers.append(_get_scheduler(name, opt, lr, total_steps, final_div_factor))

    else:
        schedulers.append(_get_scheduler(name, optimizer, lr, total_steps, final_div_factor))
