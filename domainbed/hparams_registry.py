...
def _hparams(algorithm, dataset, random_state):
    ...
    hparams["data_augmentation"] = ('aug', str(random_state.choice(['aug', 'basic'])))
    hparams["auto_da"] = ('none', str(random_state.choice(['ta', 'ra', 'uniform', 'autoaugment'])))
    hparams["da_mode"] = ('append', str(random_state.choice(['append', 'online'])))

    hparams["num_samples"] = (1, random_state.choice([1, 2, 3]))
    hparams["magnitude"] = (-1, random_state.choice(list(range(32))))
    hparams["num_ops"] = (1, random_state.choice([1, 2, 3]))
    hparams["tf_range"] = ('wide', str(random_state.choice(['default', 'wide', 'wider'])))
...
###################### DA #######################################
    ...
    if algorithm == "ERMAdv" or algorithm == "ERMDAdv":
        hparams["alpha"] = (0.5, random_state.choice([0.2, 0.5, 0.8]))
        hparams["reward"] = ('mix', random_state.choice(['mix', 'label', 'domain']))
        hparams["use_teacher"] = (True, True)
    ...
    hparams['lr'] = (5e-5, random_state.choice([1e-5, 3e-5, 5e-5]))
    hparams['weight_decay'] = (0.0, random_state.choice([1e-4, 1e-6]))
    hparams['domain_eval'] = (False, False)
