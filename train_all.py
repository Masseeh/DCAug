...
def run_trial(args, hparams):
    timestamp = misc.timestamp()
    args.unique_name = f"{timestamp}_{args.name}_{args.test_envs[0]}_{args.trial_seed}" if args.unique_name == None else args.unique_name

    # path setup
    args.work_dir = Path(".")
    args.data_dir = Path(args.data_dir)

    args.out_root = args.work_dir / Path(args.log_dir) / args.dataset
    args.out_dir = args.out_root / args.unique_name
    args.out_dir.mkdir(exist_ok=True, parents=True)

    # writer = get_writer(args.out_root / "runs" / args.unique_name, args.tb)
    writer = get_writer(args.out_dir, args.tb)
    ...
    # elif len(args.test_envs) == 1 and not isinstance(args.test_envs[0], list):
    #     args.test_envs = [args.test_envs]
    elif not isinstance(args.test_envs[0], list):
        args.test_envs = [args.test_envs]
    ...
    for test_env in args.test_envs:
        res, records = train(
            ...
            logger=logger,
            writer=writer,
            target_env=test_env[0] if args.target_env == -1 else args.target_env
        )
    ...
    ds_envs = [dataset.environments[te[0]] for te in sorted(args.test_envs, key=lambda x: x[0])]
    table = PrettyTable(["Selection"] + ds_envs + ["Avg."])
    ...
    with open(args.out_dir/'done', 'w') as f:
        f.write('done')
    
    writer.close()

    return (results, ds_envs)
...
def main():
    ...
    parser.add_argument("--target_env", type=int, default=-1)  # sketch in PACS
    ...
    hparams = Config(open("config.yaml", encoding="utf8"), default=hparams)
    hparams = dict(hparams)
    
    if len(args.configs) > 0:
        model_config = dict(Config(open(args.configs[0], encoding="utf8"))[f'{args.dataset}{args.test_envs[0]}'])
        hparams.update(model_config)
    ...
    if args.device == 'cpu':
        ...
        hparams['device'] = f'cuda:{args.device}'
